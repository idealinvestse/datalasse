...
cp "$ROOT_DIR/lib/retry.sh" "$ROOT_DIR/lib/fallback.sh" "$ROOT_DIR/lib/research-state.sh" "$TESTBIN/lib/"
cp "$ROOT_DIR/research-init" "$ROOT_DIR/research-goal" "$ROOT_DIR/research-status" "$TESTBIN/"
...
run_out=$(cd /tmp && RESEARCH_DIR="$RESEARCH_DIR" WORKSPACE_DIR="$WORKSPACE_DIR" \
  OPENROUTER_API_KEY="" EXA_API_KEY=dummy SERPER_API_KEY=dummy \
